<template>
  <div class="data-analysis-page">
    ...
    <el-row :gutter="20" style="margin-top: 20px;">
        ...
        <ComparisonChart 
          :stations="stations"
          v-model:selected-stations="comparisonStationNames" 
        />
        </el-col>
      </el-row>
  </div>
...
const comparisonStationNames = ref<string[]>([]); // 修正: comparisonStationIds -> comparisonStationNames
const mapStore = useMapStore();

const handleStationClick = (stationName: string) => { // 修正: stationId -> stationName
  if (!comparisonStationNames.value.includes(stationName)) {
    if (comparisonStationNames.value.length >= 5) {
      ElMessage.warning('最多只能选择 5 个站点进行比较');
      return;
    }
    comparisonStationNames.value.push(stationName);
  }
};
