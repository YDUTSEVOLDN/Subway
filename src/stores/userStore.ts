...
export const useUserStore = defineStore({
  ...
  actions: {
    ...
    async login(username: string, password: string): Promise<boolean> {
      try {
        const user = await authService.login(username, password); // The interceptor returns data directly
    ...
    // 用户注册
    async register(username: string, email: string, password: string, role: UserRole, verificationCode: string): Promise<boolean> {
      try {
        console.log('Registering with code:', verificationCode); // 记录验证码用于调试
        const user = await authService.register(username, email, password, role, verificationCode); // The interceptor returns data directly
        if (user && user.accessToken) {
          this.token = user.accessToken;
          this.user = user;
          localStorage.setItem('user', JSON.stringify(user));
